"""ThoughtLens design system — single source of truth for all visualisation colours.

Mirrors the palette specified in the design document so that every figure,
report tab, and terminal widget shares a coherent look.
"""
...
def heatmap_colorscale() -> list[list[float | str]]:
    """Return a Plotly-compatible colour scale for activation heatmaps."""
    return [
...
def node_color(node_type: str) -> str:
    """Return the canonical colour for a given attribution-graph node type."""
    mapping = {
...
def edge_color(weight: float) -> str:
    """Return promoting vs suppressing colour given a signed edge weight."""
    if weight >= 0:
        return THOUGHTLENS_COLORS["edge_promoting"]
    return THOUGHTLENS_COLORS["edge_suppressing"]
